...
CTEST(instruction, when_instructionToStream_then_canBeRecovered)
{
	...
	instruction_t *instruction = instruction_create(NO_OP, param1, param2);
	...
	ASSERT_EQUAL(NO_OP, recovered->icode);
...
CTEST(instruction, when_instructionListToStream_then_canBeRecovered)
{
	...
	instruction_t *exit_i = instruction_create(EXIT, param1, param2);
	instruction_t *read_i = instruction_create(READ, param1, param2);
	instruction_t *write_i = instruction_create(WRITE, param1, param2);
	...
	for (int i = 0; i < list_size(recovered); i++)
	{
		...
		ASSERT_EQUAL(EXIT + i, i_recovered->icode);
